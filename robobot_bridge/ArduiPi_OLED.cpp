...
{
//   fprintf(stdout, "# ArduiPi_OLED::fastSPIwrite(char) .h\n");
...
ArduiPi_OLED::ArduiPi_OLED() 
{
  ...
  printf("ArduiPi_OLED::ArduiPi_OLED init\n");
  // Command I/O
  rst = 0 ;
  dc  = 0 ;
  cs =  0 ;
...
{
  printf("# ArduiPi_OLED::oled_is_spi_proto is here\n");
...
//   fprintf(stdout, "# display::begin before constructor (%s)\n", oled_type_str[1]);
  constructor(oled_width, oled_height);

//   fprintf(stdout, "# display::begin after constructor\n");
  // Setup reset pin direction (used by both SPI and I2C)  
  bcm2835_gpio_fsel(rst, BCM2835_GPIO_FSEL_OUTP);
  bcm2835_gpio_write(rst, HIGH);
  
//   fprintf(stdout, "# display::begin set pins\n");
...
//   fprintf(stdout, "# display::begin gpio write\n");
  // bring out of reset
  bcm2835_gpio_write(rst, HIGH);
  ...
  else
  {
    ...
    else
    {
      ...
      if (oled_type == OLED_SH1106_I2C_128x64 or oled_type == OLED_SH1106_SPI_128x64)
        contrast = 0x80;
      else
...
//   fprintf(stdout, "# display::begin set external Vcc\n");
...
//   fprintf(stdout, "# display::begin set display off\n");
  sendCommand(SSD_Display_Off);                    
//   fprintf(stdout, "# display::begin set multiplex ratio\n");
  sendCommand(SSD_Set_Muliplex_Ratio, multiplex); 
  
//   fprintf(stdout, "# display::begin before not 96x96\n");
  if (oled_type == OLED_SEEED_I2C_96x96 )
  {
  ...
  else if (oled_type == OLED_SH1106_I2C_128x64 or oled_type == OLED_SH1106_SPI_128x64)
  {
...
//   fprintf(stdout, "# display::begin set contrast\n");
  
  sendCommand(SSD_Set_ContrastLevel, contrast);

//   fprintf(stdout, "# display::begin stop scroll\n");
  stopscroll();
  
  // Empty uninitialized buffer
  clearDisplay();
  
//   fprintf(stdout, "# display::begin turn on display\n");
  // turn on oled panel
  sendCommand(SSD_Display_On);              
  
  // wait 100ms
  usleep(100000);
//   fprintf(stdout, "# display::begin ends\n");
}
...
void ArduiPi_OLED::putSeedChar(char C)
{
    ...
    for(char i=0;i<8;i=i+2)
    {
        for(char j=0;j<8;j++)
        {
            ...
            char c=0x00;
            char bit1=( seedfont[C-32][(int)i]   >> j) & 0x01;  
...
{ 
  ...
  if (isSPI())
  {
//     fprintf(stdout, "# ArduiPi_OLED::sendCommand - SPI - DC->low\n");
    // Setup D/C line to low to switch to command mode
    bcm2835_gpio_write(dc, LOW);

//     fprintf(stdout, "# ArduiPi_OLED::sendCommand - SPI write fast\n");
    // Write Data on SPI
    fastSPIwrite(c);
  }
  // so I2C
...
//   fprintf(stdout, "# ArduiPi_OLED::sendCommand - end\n");
}
...
//   fprintf(stdout, "# ArduiPi_OLED::display - start (type = %d) isSPI=%d\n", oled_type, isSPI());
  if (oled_type == OLED_SEEED_I2C_96x96 )
  {
...
//   fprintf(stdout, "# ArduiPi_OLED::display - after send commands\n");
  uint16_t i=0 ;
...
//       fprintf(stdout, "# send 0xEE as end of line %d (%x %d)\n", k,buff[1],buff[2]);
      sendCommand(0xEE) ;//end
      }
    }
    else
    {
...
// clear everything (in the buffer) to this byte value
void ArduiPi_OLED::clearDisplay(uint8_t v) 
{
  memset(poledbuff, v, oled_buff_size);
}
